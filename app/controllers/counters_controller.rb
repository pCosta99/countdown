...
class CountersController < ApplicationController
  ...
  def show
    time = (@counter.deadline - DateTime.now).seconds
    @hours = time.in_hours
    @minutes = time.in_minutes - (@hours.floor * 60)
    @seconds = time - (@hours.floor * 3600) - (@minutes.floor * 60)
  end
  ...
  def counter_params
    params.require(:counter).permit(:name, :description, :deadline)
  end
end
